...
async function getEbayToken() {
  ...
  if (!response.ok) throw new Error('eBay token fetch failed');
...
async function getEbayBrowseData(itemName) {
  try {
    ...
    const prices = items.map(item => parseFloat(item?.price?.value)).filter(p => !isNaN(p) && p > 0);
    if (prices.length === 0) return null;
    const sorted = [...prices].sort((a, b) => a - b);
    const trimCount = Math.floor(sorted.length * 0.1);
    const trimmed = sorted.slice(trimCount, sorted.length - trimCount);
    const trimmedAvg = trimmed.length > 0 ? trimmed.reduce((a, b) => a + b, 0) / trimmed.length : prices.reduce((a, b) => a + b, 0) / prices.length;
    ...
    const avgWatchers = watchCounts.length > 0 ? watchCounts.reduce((a, b) => a + b, 0) / watchCounts.length : 0;
    const demandLevel = avgWatchers > 10 ? 'High' : avgWatchers > 3 ? 'Medium' : 'Low';
    return {
      askingAvg: Math.round(trimmedAvg * 100) / 100,
      estimatedSoldAvg: Math.round(trimmedAvg * 0.85 * 100) / 100,
...
Use $${ebayData.estimatedSoldAvg} as your sell_price_avg. Adjust for condition. Keep range between $${ebayData.lowPrice} and $${ebayData.highPrice}.`
    ...
    const data = await response.json();
    return res.status(200).json({ content: data.content[0].text, ebayData, ebaySearchTerm });
